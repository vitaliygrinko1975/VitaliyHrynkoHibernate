...
public class MySqlOrderDAO implements OrderDAO {
    private static final Logger LOG = Logger.getLogger(String.valueOf(MySqlOrderDAO.class));

    private static MySqlOrderDAO instance;

    public static synchronized MySqlOrderDAO getInstance() throws DBException {
        if (instance == null) {
    ...
    public List<Orders> findOrders() throws DBException {
        ...
        try {
            ...
            rs = stmt.executeQuery(Query.SQL_FIND_ALL_ORDERS);
            while (rs.next()) {
    ...
    public List<Orders> findOrders(int statusId) throws DBException {
        ...
        try {
            con = DBManager.getConnection();
            pstmt = con.prepareStatement(Query.SQL_FIND_ORDERS_BY_STATUS);
    ...
    public List<Orders> findOrders(User user, int statusId) throws DBException {
        ...
        try {
            con = DBManager.getConnection();
            pstmt = con.prepareStatement(Query.SQL_FIND_ORDERS_BY_STATUS_AND_USER);
            pstmt.setInt(1, statusId);
     ...
     */
        ...
        orders.setUserId(rs.getLong(Fields.ORDER_USER_ID));
