...
public class MySqlStatusDAO implements StatuseDAO {
    private static final Logger LOG = Logger.getLogger(MySqlCarsDAO.class);

    private static MySqlStatusDAO instance;

    public static synchronized MySqlStatusDAO getInstance() throws DBException {
        if (instance == null) {
    ...
    public List<UserOrderBean> getUserOrderBeans() throws DBException {
        ...
        try {
            ...
            rs = stmt.executeQuery(Querys.SQL_GET_USER_ORDER_BEANS);
            while (rs.next()) {
